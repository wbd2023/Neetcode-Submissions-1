class Solution:
    ...
    def encode(self, strs: List[str]) -> str:
        escaped = [
            string.replace(self.SEQUENCE_WORD, self.ESCAPE + self.SEQUENCE_WORD) for string in strs
        ]
        # print(escaped)
        escaped = [
            string.replace(self.SEQUENCE_EMPTY, self.ESCAPE + self.SEQUENCE_EMPTY)
            for string in escaped
        ]
        # print(escaped)
